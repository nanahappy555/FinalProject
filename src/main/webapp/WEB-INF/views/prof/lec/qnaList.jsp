				...
				<c:if test="${!empty lqList }" >
			     	...
			     	<tr>
			     		...
			     		<td class="table-title">
							...
							<c:choose>
								<c:when test="${qna.replycnt ne 0 }">
									<span class="nav-item">															
										<span class="text-success">[답변완료]</span>&nbsp;
									</span>
								</c:when>
								<c:when test="${qna.replycnt eq 0}">
									<span class="nav-item">															
										<span class="text-warning">[답변대기중]</span>&nbsp;
									</span>
								</c:when>
						...
						<!-- href="javascript:void(0);" 이거는 유효성 오류가 생길수있다고 함 -->
			     		<a ${member.memClassCd eq 2? 'href="http://www.naver.com" target="_blank"' : 'javascript:void(0);' }>
				     		이거비밀글이지롱................
			     		</a>
...
var url = "<%=request.getContextPath()%>/prof/lec/qna/detail";
